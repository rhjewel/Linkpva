...
?>
<div id="post-<?php echo esc_attr($post_id); ?>" <?php post_class('blog-card mb-4'); ?> data-blog-item data-category="<?php echo esc_attr($category_slug); ?>">
    <article class="linkpva-blog-card">
        <a class="linkpva-blog-visual <?php echo esc_attr($visual_style); ?>" href="<?php echo esc_url($permalink); ?>" aria-label="<?php echo esc_attr(sprintf(__('Read %s', 'linkpva'), $title)); ?>">
            <?php if (has_post_thumbnail($post_id)) : ?>
                <?php echo get_the_post_thumbnail($post_id, 'blog-stnd', array('loading' => 'lazy', 'decoding' => 'async')); ?>
            <?php else : ?>
